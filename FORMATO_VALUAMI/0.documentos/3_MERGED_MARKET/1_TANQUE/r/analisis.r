...
# === DATABASE === (((
sprintf("--- DATABASE ---")
datos = read.table('merged_market_tank_usd_converted_4_sep.txt',header=TRUE,na.string="NA")
X1 = datos$EDAD
X2 = datos$VOL
Y =  datos$PRECIO
sprintf("X1. EDAD")
...
# === REDUCCION MODELO === (((
sprintf(" --- REDUCCION MODELO --- ")
n = dim(anova)[1] # véase que se tienen 4 filas en la tabla anova (línea 64).
qt(c(0.05,0.95),df=anova[n,1]) # se obtiene el percentil de la dsitribución T.
summary(regresion)$coefficients
# regresion = lm(Y ~ X1+X2-1,datos)
# summary(regresion)$coefficients
# regresion = lm(Y ~ X1,datos)
# summary(regresion)$coefficients
# )))

# === SUSTITUCION === (((
sprintf(" --- SUSTITUCION --- ")
predict(regresion,data.frame(X1=c(2),X2=c(1000) ))
# )))
